...
theorem insertUnit_preserves_invariant {n : Nat} (assignments0 : Array Assignment)
  ...
  . by_cases hasAssignment l.2 assignments[l.1.1]!
    ...
    . next h3 =>
      ...
      . next i_eq_l =>
        ...
        apply Exists.intro mostRecentUnitIdx ∘ Exists.intro l.2
        apply Exists.intro i_gt_zero
        simp only [insertUnit, h3, Prod.mk.eta, ite_false, Array.get_push_eq, i_eq_l]
        constructor
        . conv => lhs; rw [← @Prod.mk.eta (PosFin n) Bool l]
        . constructor
          . rw [Array.get_modify_at_idx l_in_bounds]
            simp only [← i_eq_l, h1]
          . constructor
            . simp only [Bool.not_eq_true, getElem!, l_in_bounds, dite_true] at h3
              simp only [← i_eq_l, ← h1]
              simp only [i_eq_l, h3]
            . intro k hk
  ...
  . by_cases hasAssignment l.2 assignments[l.1.1]!
    . next h5 =>
      ...
      have j_lt_updatedUnits_size : j.1 < (insertUnit (units, assignments, foundContradiction) l).1.size := by
        ...
        exact j.2
      apply Exists.intro ⟨j.1, j_lt_updatedUnits_size⟩
      apply Exists.intro b ∘ Exists.intro i_gt_zero
      simp only [insertUnit, h5, Prod.mk.eta, ite_true]
      apply And.intro h1 ∘ And.intro h2 ∘ And.intro h3
      intro k k_ne_j
    ...
    . next h5 =>
      ...
      . next i_eq_l =>
        ...
        | true, true =>
          exfalso
          have assignments_i_rw : assignments[i.1]! = assignments[i.1] := by simp only [getElem!, i_in_bounds, dite_true]
          rw [hl, ← i_eq_l, assignments_i_rw, h2] at h5
          exact h5 (has_of_add _ true)
        | true, false =>
          apply Exists.intro ⟨j.1, j_lt_updatedUnits_size⟩
          apply Exists.intro mostRecentUnitIdx ∘ Exists.intro i_gt_zero
              ...
              . constructor
                . match h : assignments0[i.val]'_ with
                  ...
                  | neg =>
                    ...
                    simp only [hasAssignment, hl, getElem!, l_in_bounds, h2, dite_true, ite_false, not_true, Bool.not_eq_true, hasNegAssignment] at h5
                ...
                . intro k k_ne_j k_ne_l
                  ...
                  . exfalso
                    ...
                    . simp only [← k_eq_units_size, not_true] at k_ne_l
                      exact k_ne_l rfl
        | false, true =>
          apply Exists.intro mostRecentUnitIdx ∘ Exists.intro ⟨j.1, j_lt_updatedUnits_size⟩
          apply Exists.intro i_gt_zero
          simp only [insertUnit, h5, Prod.mk.eta, ite_false, Array.get_push_eq, ne_eq]
          constructor
          . simp only [i_eq_l, ← hl]
              ...
              . constructor
                . match h : assignments0[i.val]'_ with
                  ...
                  | pos =>
                    ...
                    simp only [i_eq_l] at h2
                    simp only [hasAssignment, hl, getElem!, l_in_bounds, h2, dite_true, ite_true, Bool.not_eq_true, hasPosAssignment, not_true] at h5
  ...
  . have j1_lt_updatedUnits_size : j1.1 < (insertUnit (units, assignments, foundContradiction) l).1.size := by
    ...
    apply Or.inr ∘ Or.inr ∘ Exists.intro ⟨j1.1, j1_lt_updatedUnits_size⟩
    apply Exists.intro ⟨j2.1, j2_lt_updatedUnits_size⟩
    apply Exists.intro i_gt_zero
    simp only [insertUnit, Prod.mk.eta]
    constructor
    . split
          ...
          . intro k k_ne_j1 k_ne_j2
            ...
            . next k_not_lt_units_size =>
              ...
              . next h =>
                ...
                intro l_eq_i
                simp only [getElem!, l_eq_i, i_in_bounds, h3, dite_true, Bool.not_eq_true, has_of_both] at h
                exact h ⟨⟩
...
theorem clearUnit_foldl_preserves_size {α : Type u} (assignments : Array Assignment) (f : Array Assignment → α → Array Assignment)
  ...
  have hb : assignments.size = assignments.size := by rfl
...
def clear_insert_induction_motive {n : Nat} (f : DefaultFormula n) (assignments_size : f.assignments.size = n)
  ...
  fun idx assignments => ∃ hsize : assignments.size = n, ∀ i : Fin n,
    have i_lt_assignments_size : i.1 < assignments.size := by
      rw [hsize]
      exact i.2
            ...
            . constructor
              ...
              . intro k k_ge_idx_add_one k_ne_j1
                ...
                . next h2 =>
                  ...
                  exact h3 (by rfl)
      . next idx_ne_j2 =>
        apply Or.inr ∘ Or.inr ∘ Exists.intro j1 ∘ Exists.intro j2
        apply Exists.intro i_gt_zero
        constructor
        . apply Nat.le_of_lt_succ
          rw [← Nat.succ_eq_add_one]
...
theorem confirmRupHint_preserves_invariant_helper {n : Nat} (f : DefaultFormula n) (f_assignments_size : f.assignments.size = n)
      ...
      . constructor
        ...
        . constructor
          . simp only [List.get, Bool.not_eq_true]
            simp only [getElem!, l_in_bounds, dite_true, Bool.not_eq_true] at h
  ...
  . let l' := acc.2.1.get j
    ...
    . next l_eq_i =>
      ...
      have l_ne_l' : l.2 ≠ l'.2 := by
        intro l_eq_l'
        rw [l_eq_i] at h
        simp only [getElem!, i_in_bounds, h1, ← l_eq_l', dite_true, has_of_add, not_true] at h
      by_cases l.2
      . next l_eq_true =>
          ...
          . simp only [List.get, Nat.add_eq, Nat.add_zero, List.length_cons, ne_eq]
            ...
            . constructor
              . simp only [l'_eq_false, hasAssignment, ite_false] at h2
                simp only [hasAssignment, l_eq_true, getElem!, l_eq_i, i_in_bounds, h1, addAssignment, l'_eq_false,
                  ite_false, dite_true, ite_true, hasPos_of_addNeg] at h
          ...
          . simp only [List.get, Nat.add_eq, Nat.add_zero, List.length_cons, ne_eq]
            ...
            . constructor
              . simp only [hasAssignment, l'_eq_true, ite_true] at h2
                simp only [hasAssignment, l'_eq_true, getElem!, l_eq_i, i_in_bounds, h1, addAssignment, l_eq_false,
                  ite_false, dite_true, ite_true, hasNeg_of_addPos] at h
  ...
  . have j1_succ_in_bounds : j1.1 + 1 < (l :: acc.2.1).length := by
    ...
    have l_ne_i : l.1.1 ≠ i.1 := by
      ...
      split at h
      . simp (config := {decide := true}) only [getElem!, l_eq_i, i_in_bounds, h1, dite_true] at h
      . simp (config := {decide := true}) only [getElem!, l_eq_i, i_in_bounds, h1, dite_true] at h
    constructor
    . rw [Array.get_modify_unchanged l_in_bounds i_in_bounds _ l_ne_i]
      exact h1
      ...
      . intro k k_ne_j1_succ k_ne_j2_succ
        ...
        . next k_ne_zero =>
          ...
          have k'_ne_j1 : ⟨k', k'_in_bounds⟩ ≠ j1 := by
            simp only [List.length_cons, k_eq_succ, ne_eq, Fin.mk.injEq, Nat.succ.injEq] at k_ne_j1_succ
            exact Fin.ne_of_val_ne k_ne_j1_succ
          have k'_ne_j2 : ⟨k', k'_in_bounds⟩ ≠ j2 := by
            simp only [List.length_cons, k_eq_succ, ne_eq, Fin.mk.injEq, Nat.succ.injEq] at k_ne_j2_succ
...
theorem confirmRupHint_preserves_invariant {n : Nat} (f : DefaultFormula n) (f_assignments_size : f.assignments.size = n)
  ...
  . have rupHint_clause_options :
      ...
      | some (some c) => exact (Or.inr ∘ Or.inr ∘ Exists.intro c) (by rfl)
    ...
    . simp only [rupHint_clause_eq_c]
      have reduce_c_options : reduce c acc.1 = ReduceResult.encounteredBoth ∨ reduce c acc.1 = ReduceResult.reducedToEmpty ∨
        ...
        | ReduceResult.reducedToUnit l => exact (Or.inr ∘ Or.inr ∘ Or.inl ∘ Exists.intro l) (by rfl)
...
theorem derivedLits_nodup {n : Nat} (f : DefaultFormula n) (f_assignments_size : f.assignments.size = n) (rupHints : Array Nat)
  ...
  . by_cases k.1 = i.1
    . next k_eq_i =>
      ...
      simp only [getElem_fin, derivedLits_arr_def, ne_eq] at h3
      rw [li_eq_lj] at h3
    ...
    . next k_ne_i =>
      ...
      simp (config := { decide := true }) only [getElem_fin, derivedLits_arr_def, ne_eq, Array.getElem_eq_data_get] at h3
  . by_cases li.2 = true
    . next li_eq_true =>
      have i_ne_k2 : ⟨i.1, i_in_bounds⟩ ≠ k2 := by
        intro i_eq_k2
        rw [← i_eq_k2] at k2_eq_false
        simp only [derivedLits_arr_def, getElem_fin, Array.getElem_eq_data_get, k2_eq_false] at li_eq_true
      have j_ne_k2 : ⟨j.1, j_in_bounds⟩ ≠ k2 := by
        ...
        simp only [derivedLits_arr_def, getElem_fin, Array.getElem_eq_data_get, k2_eq_false, li_eq_lj] at li_eq_true
      ...
      . next i_eq_k1 =>
        ...
        simp only [li_eq_lj] at h3
        simp only [derivedLits_arr_def, getElem_fin, Array.getElem_eq_data_get, ne_eq, not_true] at h3
      . next i_ne_k1 =>
        specialize h3 ⟨i.1, i_in_bounds⟩ i_ne_k1 i_ne_k2
        simp (config := { decide := true }) only [getElem_fin, Array.getElem_eq_data_get, ne_eq, derivedLits_arr_def] at h3
    . next li_eq_false =>
      simp only [Bool.not_eq_true] at li_eq_false
      have i_ne_k1 : ⟨i.1, i_in_bounds⟩ ≠ k1 := by
        intro i_eq_k1
        rw [← i_eq_k1] at k1_eq_true
        simp only [derivedLits_arr_def, getElem_fin, Array.getElem_eq_data_get, k1_eq_true] at li_eq_false
      have j_ne_k1 : ⟨j.1, j_in_bounds⟩ ≠ k1 := by
        intro j_eq_k1
        rw [← j_eq_k1] at k1_eq_true
        simp only [derivedLits_arr_def, getElem_fin, Array.getElem_eq_data_get] at li_eq_lj
        simp only [derivedLits_arr_def, getElem_fin, Array.getElem_eq_data_get, k1_eq_true, li_eq_lj] at li_eq_false
...
theorem restoreAssignments_performRupCheck_base_case {n : Nat} (f : DefaultFormula n) (f_assignments_size : f.assignments.size = n)
  ...
  . apply Or.inr ∘ Or.inl
    ...
    apply Exists.intro ⟨j.1, j_lt_derivedLits_arr_size⟩
    apply Exists.intro (List.get derivedLits j).2 ∘ Exists.intro i_gt_zero
    constructor
    . apply Nat.zero_le
    . constructor
  ...
  . apply Or.inr ∘ Or.inr
    ...
    apply Exists.intro ⟨j1.1, j1_lt_derivedLits_arr_size⟩
    apply Exists.intro ⟨j2.1, j2_lt_derivedLits_arr_size⟩
    apply Exists.intro i_gt_zero ∘ And.intro (Nat.zero_le j1.1) ∘ And.intro (Nat.zero_le j2.1)
    constructor
    . simp only [derivedLits_arr_def, getElem_fin, Array.getElem_eq_data_get, ← j1_eq_i]
      rw [← j1_eq_true]
      conv => lhs; rw [← @Prod.mk.eta (PosFin n) Bool (derivedLits.get ⟨j1.1, _⟩)]
    . constructor
...
theorem restoreAssignments_performRupCheck {n : Nat} (f : DefaultFormula n) (f_assignments_size : f.assignments.size = n)
  ...
  have f'_def : f' = (performRupCheck f rupHints).1 := by rfl
  ...
  have derivedLits_arr_def : derivedLits_arr = {data := derivedLits} := by rfl
