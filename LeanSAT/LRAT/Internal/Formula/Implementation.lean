...
/-- Note: This function assumes that the provided `clauses` Array is indexed according to the `clauses` field
    invariant described in the DefaultFormula doc comment. -/
...
def formulaEntails_def {n : Nat} (p : (PosFin n) → Bool) (f : DefaultFormula n) :
...
def performRupAdd {n : Nat} (f : DefaultFormula n) (c : DefaultClause n) (rupHints : Array Nat) : DefaultFormula n × Bool :=
  let negC := negate c
...
/-- Takes in a formula f and a single ratHint and attempts to verify that f is inconsistent with the negation of the ratHint's
    clause. It returns:
    ...
    Note: This function assumes that the ratUnits corresponding to this rat check have NOT already been added to f. In terms of
    input expectations and output guarantees, this function is more analogous to performRupAdd than performRupCheck -/
def performRatCheck {n : Nat} (f : DefaultFormula n) (negPivot : Literal (PosFin n)) (ratHint : Nat × Array Nat) :
...
/-- Attempts to verify that c can be added to f via unit propagation. If it can, it returns ((f.insert c), true). If it can't,
    it returns false as the second part of the tuple (and no guarantees are made about what formula is returned) -/
def performRatAdd {n : Nat} (f : DefaultFormula n) (c : DefaultClause n) (pivot : Literal (PosFin n)) (rupHints : Array Nat)
